...
async function saveCodexFiles({ configText, authText }, homeDir = os.homedir()) {
  ...
  const paths = getCodexPaths(homeDir);
  const existingAuthText = await readTextIfExists(paths.authPath);
  const resolvedAuthText = resolveAuthJsonForSave({
    configText,
    authJsonText: authText,
    existingAuthJsonText: existingAuthText
  });
  await fs.mkdir(paths.codexDir, { recursive: true });

  await Promise.all([
    fs.writeFile(paths.configPath, configText, 'utf8'),
    fs.writeFile(paths.authPath, resolvedAuthText, 'utf8')
  ]);
